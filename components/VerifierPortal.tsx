...
import { Search, CheckCircle, XCircle, Shield, FileText, Share } from 'lucide-react';
...
export default function VerifierPortal() {
    ...
    const receivedCredentials = ledger.filter(tx =>
        tx.type === 'CREDENTIAL_FORWARD' && tx.details?.verifierDid === currentUser?.did
    ...
    if (!currentUser) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] space-y-8">
                ...
                <Button onClick={() => registerUser('ORGANIZATION', 'VERIFIER', { name: 'Empoyer/Body', address: '', registrationId: '' })} className="gap-2 bg-green-600 hover:bg-green-700">
    ...
    const handleViewArtifact = (credentialJson?: string) => {
        ...
        try {
            ...
            if (submission?.details?.artifactDataArray && submission.details.artifactDataArray.length > 0) {
                ...
                <Card>
                    ...
                    <CardContent>
                        {receivedCredentials.length === 0 ? (
                            <p className="text-gray-500 text-sm text-center py-8">No credentials received yet.</p>
                        ) : (
                            <div className="space-y-4">
                                {receivedCredentials.map((tx) => (
                                    <div key={tx.dataHash} className="flex flex-col p-4 border rounded-lg hover:bg-gray-50 space-y-3">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <span className="font-medium text-gray-900">Forwarded VC</span>
                                            ...
                                            <Button
                                                variant="outline"
                                                onClick={() => {
                                                    setJsonInput(JSON.stringify(tx.details.credential, null, 2));
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-semibold text-gray-500 uppercase">Sender (Student)</p>
                                                <p className="text-xs font-mono text-gray-700 truncate">{tx.actor}</p>
                                            </div>
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-semibold text-gray-500 uppercase">Message</p>
                                                <p className="text-sm text-gray-700 italic">"{tx.details.message || 'No message attached.'}"</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-2 border-t pt-3">
                                            {(() => {
                                                ...
                                                if (submission?.details?.artifactDataArray) {
                                                    return submission.details.artifactDataArray.map((artifact: any, idx: number) => (
                                                        <Button
                                                            key={idx}
                                                            variant="ghost"
                                                        ...
                                                        >
                                                            <FileText className="w-3 h-3 mr-1" /> {artifact.filename}
                                                        </Button>
                                                    ));
                                                }
                                                ...
                                                return (
                                                    <Button
                                                        variant="ghost"
                                                        onClick={() => handleViewArtifact(JSON.stringify(tx.details.credential))}
                                                        className="text-xs text-blue-600 h-8"
                                                    >
